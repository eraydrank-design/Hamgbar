  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <header className="flex items-end justify-between border-b border-white/10 pb-6">
        <div>
          <h1 className="font-serif text-3xl font-bold text-gradient-gold mb-2">Announcements</h1>
          <p className="text-muted-foreground">News, events, and updates from the HangBar team.</p>
        </div>
        {isAdmin && !isAdding && (
          <button
      ...
      <AnimatePresence>
        {isAdding && (
          ...
          >
            ...
            <h3 className="font-serif text-xl font-bold text-foreground mb-4">New Announcement</h3>
            <div className="space-y-4 relative z-10">
              ...
              <div className="flex items-center justify-between pt-2">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <div className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${formPinned ? 'bg-primary border-primary' : 'border-white/20 group-hover:border-primary/50 bg-black/50'}`}>
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          <div className="text-center py-20 glass rounded-2xl border-dashed">
            ...
            <p className="text-muted-foreground text-sm">It's quiet in here. Check back later.</p>
          </div>
        ) : (
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground uppercase tracking-wider">
                    ...
                    <span>•</span>
                    <span>{ann.createdAt?.toDate ? format(ann.createdAt.toDate(), 'MMMM d, yyyy') : 'Just now'}</span>
                  </div>
                </div>
